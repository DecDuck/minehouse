...
use crate::{
    ...
    mwms::{
        endpoints::{StorageEndpoints, bulk::BulkStorage, putaway::PutawayStorage},
        planner::Planner,
...
impl Planner {
    pub async fn index_regions(&self) -> Result<(), anyhow::Error> {
        ...
        while let Some((result, region_id)) = promises.next().await {
            ...
            self.state.db.sync_region_containers(region_id, &data.output_containers).await?;
        }
